...
final class DUPX_Ctrl_S0
{
    ...
    public static function stepHeaderLog(): void
    {
        ...
        $enabledAddons = InstAddonsManager::getInstance()->getEnabledAddons();
        Log::info(str_pad('ADDONS', $labelPadSize, '_', STR_PAD_RIGHT) . ': ' . (count($enabledAddons) > 0 ? implode(', ', $enabledAddons) : 'none'));
        ...
        if (InstState::isImportFromBackendMode() || InstState::isRecoveryMode()) {
            ...
            Log::info(str_pad('LICENSE', $labelPadSize, '_', STR_PAD_RIGHT) . ': ' . apply_filters('duplicator_installer_license_string', 'unknown'));
            Log::info("********************************************************************************");
        }
        ...
        $log .= str_pad('LICENSE', $labelPadSize, '_', STR_PAD_RIGHT) . ': ' . apply_filters('duplicator_installer_license_string', 'unknown') . "\n";
